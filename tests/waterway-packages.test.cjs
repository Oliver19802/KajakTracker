...
test('generated package passes the client validators and exact checksums',async()=>{
  ...
  try {
    ...
    for(const p of m.parts){const bytes=fs.readFileSync(path.join(dir,p.file));assert.equal(bytes.length,p.bytes);assert.equal(crypto.createHash('sha256').update(bytes).digest('hex'),p.sha256);validatePart(JSON.parse(bytes),p);}
